	...
	class CPK_WPCSV {
		...
		public function admin_menus( ) {
			...
			if ( function_exists( 'add_menu_page' ) && function_exists( 'add_submenu_page' ) ) {
				add_menu_page( __( 'Elevate CSV' ), __( 'Elevate CSV' ), $capability, 'wpcsv-settings', Array( $this, 'settings_page' ), NULL, NULL );
		...
		public function old_settings_page( ) {
			echo "<h1>Elevate CSV Has Moved!</h1><p>There is now a main menu item called WP CSV with 'settings', 'import', and 'export' sub menus.</p>";
		}
			...
			if ( empty( $_POST ) && empty( $_FILES ) ) {
				...
				if ( !isset( $options['error'] ) || empty( $options['error'] ) ) {
					...
					if ( file_exists( $file_destination ) ) {
						...
						if ( $line[0] != 'wp_ID' ) {
							$error = "<p>The file uploaded does not seem to be a valid Elevate CSV import file.  Please note that the column heading format changed in version 1.7.0! ('ID' became 'wp_ID', etc).</p> ";
